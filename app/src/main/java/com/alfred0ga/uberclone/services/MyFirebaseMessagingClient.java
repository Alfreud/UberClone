package com.alfred0ga.uberclone.services;
...
import com.alfred0ga.uberclone.activities.driver.NotificationBookingActivity;
import com.alfred0ga.uberclone.channel.NotificationHelper;
import com.alfred0ga.uberclone.receivers.AcceptReceiver;
import com.alfred0ga.uberclone.receivers.CancelReceiver;
...
public class MyFirebaseMessagingClient extends FirebaseMessagingService {
        ...
        if (title != null) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                ...
                if (title.contains("SOLICITUD DE SERVICIO")) {
                    ...
                    String min = data.get("min");
                    String distance = data.get("distance");
                    showNotificationAction(title, body, idClient);
                    showNotificationActivity(idClient, origin, destination, min, distance);
                } else if (title.contains("VIAJE CANCELADO")) {
                    NotificationManager manager = (NotificationManager) getSystemService(Context.NOTIFICATION_SERVICE);
                    manager.cancel(2);
    ...
    private void showNotificationApiOreo(String title, String body) {
        PendingIntent intent = PendingIntent.getActivity(getBaseContext(), 0, new Intent(),
